...
import com.xdxiaoran.VoteSystemApplication;
import com.xdxiaoran.pojo.WXChatRoom;
import com.xdxiaoran.pojo.WXMsg;
...
public class WXServerListener extends WebSocketClient {
    ...
    private static  final List<WXChatRoom> rooms=new ArrayList<>();
    ...
    //获取所有群组
    public List<WXChatRoom> getRooms(){
        return rooms;
    }
...
//        System.out.println(jsonObject.get("type").toString().equals("5000"));
//        List<Map> list = JSONArray.parseArray(, Map.class);
        if(jsonObject.get("type").toString().equals("5000")) {
            List<Map> list = JSONArray.parseArray(jsonObject.get("content").toString(), Map.class);
            for(Map<Object,Object>map:list){
                if(map.get("wxid").toString().contains("@chatroom")){
                    rooms.add(new WXChatRoom(map.get("wxid").toString(),map.get("name").toString()));
                }
            }
        }
    }
...
//        restartListener();
    ...
    public void restartListener(){
        ...
        threadPool.execute(() -> {
            VoteSystemApplication.context.close();
            VoteSystemApplication.context = SpringApplication.run(VoteSystemApplication.class,
                    VoteSystemApplication.args);
        });
        threadPool.shutdown();
    }
}
